...
class FDSA(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        shape = item_emb.shape
        lam, theta = EulerTrans(item_emb)
        item_emb = torch.stack([lam * torch.cos(theta + rotary_embedding), lam * torch.sin(theta + rotary_embedding)], dim = -1)
        item_emb = item_emb.reshape(*shape)
        ...
        shape = feature_emb.shape
        lam, theta = EulerTrans(feature_emb)
        feature_emb = torch.stack([lam * torch.cos(theta + rotary_embedding), lam * torch.sin(theta + rotary_embedding)], dim = -1) 
        feature_emb = feature_emb.reshape(*shape)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == "BPR":
            ...
            loss = self.loss_fct(logits, pos_items) + self.item_trm_encoder.loss + self.feature_trm_encoder.loss
            return loss
