...
class TransNet(nn.Module):
    def __init__(self, config, dataset):
        ...
        self.rotary_embedding = nn.Embedding(dataset.field2seqlen[config["ITEM_ID_FIELD"] + config["LIST_SUFFIX"]], self.hidden_size // 2)
    ...
    def forward(self, item_seq, item_emb):
        ...
        lam, theta = EulerTrans(input_emb)
        input_emb = torch.stack([lam * torch.cos(theta + rotary_embedding), lam * torch.sin(theta + rotary_embedding)], dim = -1)
...
class CORE(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        nn.init.zeros_(self.net.rotary_embedding.weight)
    ...
    def calculate_loss(self, interaction):
        ...
        return loss + self.net.trm_encoder.loss
