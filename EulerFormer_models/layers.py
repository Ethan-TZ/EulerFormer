...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        position_ids = torch.arange(
            input_tensor.shape[-2], dtype=torch.long, device=input_tensor.device
        )
        mixed_query_layer = self.query(input_tensor) 
        mixed_key_layer = self.key(input_tensor) 
        mixed_value_layer = self.value(input_tensor) 
        
        mixed_query_layer, mixed_key_layer = self.euler(mixed_query_layer, 'queryro'), self.euler(mixed_key_layer)
        self.get_loss(mixed_query_layer, mixed_key_layer, self.tep, self.lamb)
    ...
    def get_loss(self, q, k, tep = 1, lamb = 1e-5):
        pq, pk = EulerFormer.get_polar(q), EulerFormer.get_polar(k)
        def cal(pq, pk):
            denominator = (torch.cos(pq) * self.w) @ torch.cos(pk).transpose(-2, -1) + (torch.sin(pq) * self.w) @ torch.sin(pk).transpose(-2, -1)
            numerator = torch.diagonal(torch.exp(denominator / tep), dim1=-1, dim2=-2)
            denominator = torch.sum(torch.exp(denominator/ tep), dim= (-1)) + 1e-5
            return torch.mean(-torch.log(numerator / denominator)) * lamb
        
        self.loss = cal(pq, self.dp(pq)) + cal(pk, self.dp(pk))
